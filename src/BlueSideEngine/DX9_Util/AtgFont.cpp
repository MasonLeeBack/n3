...
HRESULT Font::Create( const CHAR* strFontFileName )
{
	...
	// mruete; prefix bug 482: added intermediate value and check
	D3DTexture * pTexture = m_xprResource.GetTexture("FontTexture");
	if ( !pTexture )
		return E_FAIL;

    return Create( pTexture, m_xprResource.GetData("FontData") );
}
                         ...
                         FLOAT fMaxPixelWidth )
{
    ...
    while( *strText )
    {
        ...
        g_pd3dDevice->SetPixelShaderConstantF( 0, pShaderConsts0, 1 );
        g_pd3dDevice->SetPixelShaderConstantF( 1, pShaderConsts1, 1 );
        ...
        g_pd3dDevice->DrawPrimitiveUP( D3DPT_QUADLIST, 1, v, sizeof(v[0]) );
    }
...
VOID Font::DrawText( FLOAT fOriginX, FLOAT fOriginY, DWORD dwColor,
						...
						FLOAT fMaxPixelWidth )
{
	WCHAR wcConvert[MAX_PATH]={0x00,};
	MultiByteToWideChar(CP_ACP, 0, strText, strlen(strText), wcConvert, MAX_PATH);
	DrawText(fOriginX, fOriginY, dwColor, wcConvert, dwFlags, fMaxPixelWidth);
}
