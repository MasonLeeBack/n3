...
VOID Input::GetInput( GAMEPAD* pGamepads )
{
    ...
    for( DWORD i=0; i < XUSER_MAX_COUNT; i++ )
    {
        ...
        pGamepads[i].fX1 = ConvertThumbstickValue( pGamepads[i].sThumbLX );
        pGamepads[i].fY1 = ConvertThumbstickValue( pGamepads[i].sThumbLY );
        pGamepads[i].fX2 = ConvertThumbstickValue( pGamepads[i].sThumbRX );
        pGamepads[i].fY2 = ConvertThumbstickValue( pGamepads[i].sThumbRY );
...
GAMEPAD* Input::GetMergedInput( DWORD dwMask )
{
    ...
    for( DWORD i=0; i < XUSER_MAX_COUNT; i++ )
    {
        if( m_Gamepads[i].bConnected && ( !dwMask || ( dwMask & ( 1 << i ) ) ) )
        {
            ...
            if( m_Gamepads[i].sThumbLX > INPUT_DEADZONE ||
                m_Gamepads[i].sThumbLX < -INPUT_DEADZONE )
                sThumbLX = m_Gamepads[i].sThumbLX;
            if( m_Gamepads[i].sThumbLY > INPUT_DEADZONE ||
                m_Gamepads[i].sThumbLY < -INPUT_DEADZONE )
                sThumbLY = m_Gamepads[i].sThumbLY;
            if( m_Gamepads[i].sThumbRX > INPUT_DEADZONE ||
                m_Gamepads[i].sThumbRX < -INPUT_DEADZONE )
                sThumbRX = m_Gamepads[i].sThumbRX;
            if( m_Gamepads[i].sThumbRY > INPUT_DEADZONE ||
                m_Gamepads[i].sThumbRY < -INPUT_DEADZONE )
