...
//	LightColor+=0.2f;
	...
	float4 result = (IN.diffAmbColor+float4(IN.pointValue,0))*diffuseTexture + float4(IN.specularColor.xyz*glossTexture.xyz*reflectpower*envTexture.xyz, 0.f);
	result.xyz*=ApplyWorldLightMap(LightColor);
	result.xyz = lerp(fogColor, result, IN.FogValue);
	result.xyz += IN.pointValue;
	return result;
}
