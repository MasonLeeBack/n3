...
//------------------------------------
vertexOutput0 VS_TransformAndTexture_0(vertexInput IN)
{
	vertexOutput0 OUT;
	OUT.hPosition = mul( float4(IN.position.xyz , 1.0) , worldViewProj);

	float4 N = mul(IN.normal, worldviewInverseTranspose); //normal vector at View space
	...
	// 1 texture => 8 m
	// 1/ 800cm
	OUT.Layer4UV = 0.002f*IN.position.xz;

	OUT.BlendUV.xy = IN.texUV;
	...
	OUT.TexCoord0 = mul(float4(IN.position.xyz , 1.f), shadowAll);
...
// No Shadow Map
vertexOutput1 VS_TransformAndTexture_1(vertexInput IN)
{
    vertexOutput1 OUT;
    OUT.hPosition = mul( float4(IN.position.xyz , 1.0) , worldViewProj);
...
{
	float3 fTemp;
	fTemp = ((fShadow.xxx*0.8f + 0.2f)*fLightMap.xyz) * 0.7f + 0.3f;
	return fTemp;
}
