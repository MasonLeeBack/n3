...
VS_OUTPUT_BLUR VS_BlurV(float3 Position : POSITION,
								   float2 TexCoord : TEXCOORD0)
{

	VS_OUTPUT_BLUR OUT = (VS_OUTPUT_BLUR)0;
	float2 texelSize = BlurWidth / ScreenSize;
	float2 s = TexCoord-texelSize*(7-1)*0.5f*float2(0,1);
...
half4 PS_BloomV(VS_OUTPUT_BLUR IN) : COLOR
{
	half4 c = 0;
...
technique Downsample
{
    pass p0
    {
		...
		PixelShader  = compile ps_2_0 PS_BloomV();
    }
}