...
float fWvalue  									: fx_custom0
 = 0.5f;
 float fCustom  									: fx_custom1
 = 0.9f;
  float fCustom2  								: fx_custom2
 = 0.5f;
...
float4 PS_Textured( vertexOutput IN): COLOR
{
	...
	float3 texCoord = float3(  IN.texCoordDiffuse.xy,fWvalue);
	float3 N = 2.0f * tex3D( normalSampler, texCoord ).xyz - 1.0f;
	float nAlpha = tex3D( normalSampler, texCoord ).w;
	float3 L = -normalize(IN.L);
	float3 R = reflect(-normalize(IN.E), N);
	
	//float4 diffMap = tex2D( diffuseSampler, IN.texCoordDiffuse);
	float4 result = tex2D( ScreenSampler, IN.texCoordDiffuse2+ N.xy * 0.32f);
	N = N * 2.0f;
	float a = 1.0f;
	...
	float spec = fCustom2 * 20.0f;
	float4 I = col * max(0 , dot(N,L) ) * 0.1f +  pow(max( 0, dot(R,L) ), spec ) * fCustom;
