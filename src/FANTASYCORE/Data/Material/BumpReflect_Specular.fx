...
struct vertexOutput {
    ...
    float3 FogValue		: TEXCOORD6;		/* x=fog, yz=worldlightmap coord */
    float3	pointValue	: TEXCOORD7;
};
...
vertexOutput mainVS_1(appdata1 IN)
{
    ...
    OUT.pointValue = ApplyPointLight( Pos, OUT.ViewNormal);	
...
//	float4 lighting = lit(dot(Ln,Nb),dot(Hn,Nb),SpecExpon);
	...
	float4 result = (lightAmbient*materialAmbient + lightColor*materialDiffuse*d +float4(IN.pointValue*4,0))*diff + (lightSpecular*materialSpecular)*spec*s  ;
