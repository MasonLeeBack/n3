...
public:
	...
	// ----- MUTATORS ---------------------
	void Initialize(const D3DXVECTOR3& PointA, const D3DXVECTOR3& PointB, const D3DXVECTOR3& PointC);
	void ComputeCellData();
	bool RequestLink(const D3DXVECTOR3& PointA, const D3DXVECTOR3& PointB, NavigationCell* Caller);
	void SetLink(CELL_SIDE Side, NavigationCell* Caller);
	void UnlinkCell(NavigationCell* Caller);
	...
	const D3DXVECTOR3& Vertex(int Vert)const;
	...
	// 실시간으로 push_back, erase를 하지 않으므로 필요하다.
	void SetUse( bool bUse )	{ m_bUse = bUse; }
	bool IsUse()				{ return m_bUse; }

	bool Save( BStream* stream );
	bool Load( BStream* stream );

	NavigationCell* GetLinkCell( int nIndex )	{ return m_Link[nIndex]; };				// 저장할 때 필요
	void SetLinkCell( NavigationCell* Cell, int nIndex )	{ m_Link[nIndex] = Cell; }	// 로딩할 때 필요
...
private:
	...
	D3DXVECTOR3 m_Vertex[3];		// pointers to the verticies of this triangle held in the NavigationMesh's vertex pool
...
//
//-------------------------------------------------------------------------------------://
inline void NavigationCell::Initialize(const D3DXVECTOR3& PointA, const D3DXVECTOR3& PointB, const D3DXVECTOR3& PointC)
{
...
inline void NavigationCell::ComputeCellData()
{
	...
	D3DXVECTOR2 Point1(m_Vertex[VERT_A].x,m_Vertex[VERT_A].z);
	D3DXVECTOR2 Point2(m_Vertex[VERT_B].x,m_Vertex[VERT_B].z);
	D3DXVECTOR2 Point3(m_Vertex[VERT_C].x,m_Vertex[VERT_C].z);
	...
	m_WallDistance[0] = D3DXVec3Length( &WallVector );

	WallVector = m_WallMidpoint[1] - m_WallMidpoint[2];
	m_WallDistance[1] = D3DXVec3Length( &WallVector );

	WallVector = m_WallMidpoint[2] - m_WallMidpoint[0];
	m_WallDistance[2] = D3DXVec3Length( &WallVector );
...
//
//-------------------------------------------------------------------------------------://
inline bool NavigationCell::RequestLink(const D3DXVECTOR3& PointA, const D3DXVECTOR3& PointB, NavigationCell* Caller)
{
...
//
//-------------------------------------------------------------------------------------://
inline void NavigationCell::UnlinkCell(NavigationCell* Caller)
{
	for ( int i=0; i<3; i++ ) {
		if ( m_Link[i] == Caller ) {
			m_Link[i] = NULL;
...
inline const D3DXVECTOR3& NavigationCell::Vertex(int Vert)const
{
