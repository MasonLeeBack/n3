...
const char* g_pszLeftBrace				= "{";
const char* g_pszRightBrace				= "}";
const char* g_pszPercentage				= "%";
...
bool Parser::Create(bool bReserveCommonOperators)
{
	...
	m_OperatorVector.push_back(g_pszLeftBrace);
	m_OperatorVector.push_back(g_pszRightBrace);
	m_OperatorVector.push_back(g_pszPercentage);
	...
	m_KeywordVector.push_back(g_pszEnum);
...
void Parser::ReserveOperator(const string &sOperator)//aleksger: prefix bug 851: Pass strings by reference
{
	m_OperatorVector.push_back(sOperator);
	sort(m_OperatorVector.begin(), m_OperatorVector.end());
}

void Parser::ReserveKeyword(const string &sKeyword)//aleksger: prefix bug 852: Pass strings by reference
{
...
bool Parser::ProcessSource(const std::string &sDirname, const std::string & sFilename, TokenList* pTokenList)//aleksger: prefix bug 853: Pass strings by reference
{
	m_sDirname = sDirname;
	...
	char buf[MAX_PATH];
	sprintf( buf, "%s%s", sDirname.c_str(), sFilename.c_str() );

	DWORD dwFileSize;
	VOID *pData;

	if( FAILED(CBsFileIO::LoadFile( buf, &pData, &dwFileSize ) ) )
	{
		HandleError("%s   D2\n", sDirname.c_str());
	...
	if(!ProcessSource((const char*)pData, pTokenList, (int)dwFileSize))
	{
...
void Parser::MakeReadyForFastSearchString()
{
	int i, nSize;
	...
	nSize = m_OperatorVector.size();

	for( i = 0; i < nSize; i++) {
		const char *pString = m_OperatorVector[i].c_str();
		unsigned char nIndex = pString[0];
		m_FastSearchString [ nIndex ].push_back( pString );
	}
}

bool Parser::ProcessSource(const char* const szStream, TokenList* pTokenList, int cCount)
{
	BsAssert(szStream);
	BsAssert(pTokenList);
	...
	MakeReadyForFastSearchString();

	const char *pStream = szStream;
	while(cCount>0)
	{
		m_iCurrentLineNumber++;
		m_iTotalLinesParsed++;

		int lineLength = ExtractLine( pStream, cLineBuffer, cCount < MAX_LINE_LENGTH ? cCount : MAX_LINE_LENGTH );
		
		pStream += lineLength;
		cCount -= lineLength;

		ParseLine(cLineBuffer);
		if(m_bMarkEndOfLine)
		{
...
bool Parser::ParseLine(const string& sLineBuffer)
{
	...
	// XXX
	string* sTokenBuffer=new string;
//	string sTokenBuffer;
	...
	unsigned int iLastMatchPosition = 0;
	...
	int lineLength = sLineBuffer.length();
	...
	while(iCurrentPosition <= lineLength)
	{
		...
		else
		{
			...
			do
			{
				...
				if((iCurrentPosition + iCopiedCharacters) > lineLength)
				{
				...
				size_t iBufferReturn = sLineBuffer.copy(cTempBuffer, iCopiedCharacters, iCurrentPosition);
				BsAssert(iBufferReturn == iCopiedCharacters); //aleksger: prefix bug 855: Make sure that we always copy everything
...
// XXX
				...
				bool bFound  = false;	
				const char *pString = sTokenBuffer->c_str();

				int i, nSize;

				unsigned char nFastIndex = pString[0];
				nSize = m_FastSearchString[ nFastIndex ].size();

				for( i = 0; i < nSize; i++) {
					if( strcmp(m_FastSearchString[ nFastIndex ][i], pString) == 0 ) {
                        bFound = true;
						break;
					}
				}
...
bool Parser::ProcessHeaders(TokenList* pTokenList)
{
	...
	TokenList::iterator itor = pTokenList->begin();
	while(1)
	{
		if( bCheckFirst )
		{
		...
		case Token::STRING:
			if(bPreProcessorSymbolFound)
			{
				if(bIncludeFound)
				{
					...
					if(!ProcessSource(m_sDirname, itor->GetString(), &IncludeTokenList))
...
bool Parser::ProcessMacros(TokenList* pTokenList)
{
	...
	while( 1 )
	{
		...
		case Token::KEYWORD:
			...
			else if(strcmp(itor->GetKeyword(), g_pszEnum)==0) {
// enum ì²˜ë¦¬
				PreProcBegin=itor;
				int enumIndex=0;
				...
				while(1) {
					itor++;
					if(itor->IsOperator() && strcmp(itor->GetOperator(), "{")==0) {
						break;
					}
				}
				CurrentMacro.Clear();
				...
				while(1) {
					itor++;
					while(itor->IsOperator() && strcmp(itor->GetOperator(), "/n")==0) {
						itor++;
					}

					if(itor->IsOperator()) {
						if(strcmp(itor->GetOperator(), "}")==0) {
							itor++;
							if(!(itor->IsOperator() && (strcmp(itor->GetOperator(), ";")==0))) {
								itor--;
							}
						...
						else if(strcmp(itor->GetOperator(), "//")==0) {
							itor++;
							while(itor->IsOperator() && strcmp(itor->GetOperator(), "/n")==0) {
								itor++;
							}
					...
					if(strcmp(itor->GetOperator(), "=")==0) {
						...
						Token tok;
						enumIndex=itor->GetInteger();
						tok.CreateInteger(enumIndex);
						CurrentMacro.m_MacroList.push_back(tok);
						m_MacroSet.insert(CurrentMacro);
						CurrentMacro.Clear();
						enumIndex++;
		...
		case Token::VARIABLE:
			...
			if(!bDefineMacro)
			{
				...
				{
					if(!ExpandMacro(pTokenList, itor, itr, bCheckFirst))
		...
		{
			if(itor!=pTokenList->end() &&
				(itor->GetType() == Token::OPERATOR)) {
				const char* str=itor->GetOperator();
				if(strcmp(str, g_pszNewLine)==0) {
...
bool Parser::ExpandMacro(TokenList* pTokenList, TokenListItor& itor, MacroSetItor itr, bool& bCheckFirst )
{
...
bool Parser::MakeToken(const string& sToken, Token::TOKEN_TYPE Type)
{
	...
	{	
		...
		if (!m_CurrentFileStack.empty())
            m_pTokenList->GetFileIndex(m_CurrentFileStack.top());
	}
	return bSuccess;
}
...
/*
	for(unsigned int i = 0; i < sToken.length(); ++i)
		if(!IsDigit(sToken[i]))
			if(!((i == 0) && (sToken[i] == '-')) && (sToken[i] != '.'))
				return false;
*/

// -8.37426e-007 ê³?ê°™ì? ?‘ì??˜ë„ ?½ê²Œ ?´ì???
	
	for(unsigned int i = 0; i < sToken.length(); ++i)
		if(!IsDigit(sToken[i])) {
			if( !(  (i == 0) && (sToken[i] == '-')  ) && 
				(  sToken[i] != '.'  ) && 
				!(  (i > 0) && (sToken[i] == 'e') && ((sToken[i+1] == '+') || (sToken[i+1] == '-')) &&  IsDigit(sToken[i-1])  ) &&
				!(  (i > 0) && (sToken[i-1] == 'e') && ((sToken[i] == '+') || (sToken[i] == '-')) &&  IsDigit(sToken[i+1])  )
			)
				return false;
		}
	return true;
...
bool Parser::IsHex(const string& sToken) const
{
	...
	if(!((sToken[0] == '0') && ((sToken[1] == 'x') || (sToken[1] == 'X'))))
		return false;
	for(unsigned int i = 2; i < sToken.length(); ++i)
...
bool Parser::HandleError(const char* lpszFormat, ...)
{
	...
	if( lpszFormat )
	{
		nBuf = _vsnprintf(szBuffer, 512, lpszFormat, args); //aleksger: prefix bug 856: Using safe CRT and it's normal chars.
