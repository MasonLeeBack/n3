...
if( m_bEvent )
{
	if( m_nIndex >= 1425 && m_nIndex < 1427 )
	{
//		DebugString( "AddCell\n" );
		int x = 10;
		x += 10;
...
bool NavigationCell::Save( BStream* stream )
{
//	m_CellPlane.Save( stream );
	for( int i=0; i<3; i++ )
	{
		stream->Write( &(m_Vertex[i].x), sizeof(float), 4 );
		stream->Write( &(m_Vertex[i].y), sizeof(float), 4 );
		stream->Write( &(m_Vertex[i].z), sizeof(float), 4 );
	}
/*
	stream->Write( &(m_CenterPoint.x), sizeof(float), 4 );
	stream->Write( &(m_CenterPoint.y), sizeof(float), 4 );
	stream->Write( &(m_CenterPoint.z), sizeof(float), 4 );
*/

	int nEvent = (int)m_bEvent;
	stream->Write( &nEvent, sizeof(int), 4 );

/*
	for( int i=0; i<3; i++ )
	{
		m_Side[i].Save( stream );
	}
*/
	return true;
}

bool NavigationCell::Load( BStream* stream )
{
//	m_CellPlane.Load( stream );
	for( int i=0; i<3; i++ )
	{
		stream->Read( &(m_Vertex[i].x), sizeof(float), 4 );
		stream->Read( &(m_Vertex[i].y), sizeof(float), 4 );
		stream->Read( &(m_Vertex[i].z), sizeof(float), 4 );
	}
...
//	stream->Read( &(m_CenterPoint.x), sizeof(float), 4 );
//	stream->Read( &(m_CenterPoint.y), sizeof(float), 4 );
//	stream->Read( &(m_CenterPoint.z), sizeof(float), 4 );

	int nEvent;
	stream->Read( &nEvent, sizeof(int), 4 );
	m_bEvent = ( nEvent != 0 );

	for( int i=0; i<3; i++ )
	{
//		m_Side[i].Load( stream );
	}
