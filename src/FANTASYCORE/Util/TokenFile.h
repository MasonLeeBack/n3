...
public:
	...
	bool Write(const std::string & sFilename, TokenList* pTokenList, DWORD dwFlags = 0); //aleksger: prefix bug 875: Pass strings by reference

	bool Read(std::istream &Input, TokenList* pTokenList);
	bool Read(const std::string & sFilename, TokenList* pTokenList); //aleksger: prefix bug 877: Pass strings by reference
...
private:
	...
	// Basic types
	void WriteString(const std::string &sData); //aleksger: prefix bug 876: Pass strings by reference
	void WriteDword(DWORD dwData);
	void WriteWord(WORD wData);
	void WriteByte(BYTE byteData);
	void WriteFloat(float dData);
