...
class NxFluidDesc
	...
	*/ 
	...
	/**
	Not available in the current release.
	Force applied to all particles at all time steps.
	Useful to simulate smoke or fire.
	*/ 
	NxVec3						externalForce;
	...
	*/ 
	...
	/**
	Not available in the current release.
	Defines the fluid's factor for smoothing impulse transfer, 
	used for collision with dynamic actors. 
	Must be between 0 and 1.
	*/ 
	NxReal						dynamicActorReactionSmoothing;

	/**
	Not available in the current release.
	Defines the fluid's factor for scaling impulse transfer, 
	used for collision with dynamic actors. 
	*/ 
	NxReal						dynamicActorReactionScaling ;
...
NX_INLINE void NxFluidDesc::setToDefault()
	...
	dynamicActorReactionSmoothing = 0.75f;
	dynamicActorReactionScaling = 0.5f;
...
NX_INLINE bool NxFluidDesc::isValid() const
	...
	if (dynamicActorReactionSmoothing  < 0.0f || dynamicActorReactionSmoothing  > 1.0f) return false;
	...
	if (maxParticles > 0xffff) return false;
	if (emitters.size() > 0xffff)
		return false;
